...
- `summary`：必须以方括号任务类型开头，可按素材角色选，多种关系用 ` + ` 拼接，不要发明新前缀：
  ...
  - 参考视频关闭时：不要写 `video editing` / `video continuation`
  - 参考视频开启时：必须绑定 `<Video N>`；summary 仍以 `[reference generation]` 为主，不要默认写成 `[video continuation]`
...
- [ ] 每个 `<Picture N>` 都在图清单里
- [ ] 参考视频开启时每个 `<Video N>` 都在视频清单里；关闭时没有 `<Video N>`
