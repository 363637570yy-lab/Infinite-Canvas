...
- 时长只用于对齐行的 `S.SS`。不要在描写里写「一段时长10秒」「竖屏 9:16」「720p」「24fps」。画幅、时长、分辨率由出片节点提交。
- 成片画幅常跟首帧图，不要在词里改比例，也不要把竖图「转成横屏」写进正文。
- 动作按真实速度安排。超时长就删动作，不要注水或慢动作。
...
全程单镜头，首尾帧都写 Shot 1；`S.SS` 用用户给出的时长秒数，必须两位小数（15 秒写成 `15.00`）：
...
- 全程单镜头，禁止切镜：正文不写 `At MM:SS.mmm` 时间码，不写 `the camera cuts to`，不写 `[Shot 2]`。对齐行已声明首尾帧都属于 Shot 1，切镜会让尾帧对齐错位。动作太多就删动作，或拆成多次出片，不要塞进一条提示词。
...
- `overall_soundscape` 用 1–4 句写环境/动作音，对白和 BGM 不进这一段。
...
11. 全程单镜头：正文禁止 `At MM:SS.mmm` 时间码、`the camera cuts to` 和 `[Shot 2]`。
...
- [ ] 全程单镜头：正文没有 At MM:SS.mmm、cuts to 或 [Shot 2]
- [ ] 帧锚点只用 Picture 1/2
