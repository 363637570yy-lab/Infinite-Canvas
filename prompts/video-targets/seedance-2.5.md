...
描写语言以用户消息里的「生成语言」为准。`@Image N is the first frame` 等声明行保持英文；声明之后的连续事件跟随生成语言。英文样例只示范结构。
...
声明之后用自然语言写一件连续发生的事（跟随生成语言）：谁在哪、怎么动、镜头怎么跟、环境声和台词。有首尾帧时补一句：画面从 `@Image 1` 的首帧自然走到 `@Image 2` 的尾帧，中间保持身份、道具归属和空间关系。

- 多镜头用 `then the camera cuts to ...`（中文可写「然后镜头切到」），不写时间码、不写 `[Shot N]`。
- 台词：`The consort says: "臣妾参见皇上。"` 或中文「妃子说：“臣妾参见皇上。”」。中文原文一字不改。只准原样保留或按时长舍弃。15 秒以内建议不超过 2 句。
- 总长控制在 700 词以内（中文按汉字计）。
