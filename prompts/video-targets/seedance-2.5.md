...
按官方四段写，全文 700 词以内（中文按汉字计），不要写成 H3 六段，也不要加 `subject_definitions` 这类字段名。
...
- 每个时间段或镜头写「起始状态 → 一个主要动作或变化 → 可见结果」，下一段从上一段的结果继续；人物、道具和站位不得在段间跳变。
...
台词按约 3–4 字/秒估算时长，必须能在所在时间段或镜头里自然说完；说不完就整句舍弃，不要压缩改写。
