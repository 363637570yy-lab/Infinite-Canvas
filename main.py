...
# 苍元算力（ai.cangyuansuanli.cn）的 seedance 系视频合同，同样是 POST /v1/videos +
# GET /v1/videos/{id}，但字段名与 chre3 完全不同（aspect_ratio / resolution /
# reference_image_urls 而不是 size / image_refs），所以单独成一个协议。
CANGYUAN_VIDEO_PROTOCOL = "cangyuan"
CANGYUAN_VIDEO_DEFAULT_MODEL = "seedance-2.0"
CANGYUAN_VIDEO_ASPECT_RATIOS = {"16:9", "9:16", "1:1", "21:9", "3:4", "4:3"}
CANGYUAN_VIDEO_RESOLUTIONS = {"480p", "720p"}
CANGYUAN_VIDEO_DEFAULT_RESOLUTION = "720p"
CANGYUAN_VIDEO_MIN_DURATION = 4
CANGYUAN_VIDEO_MAX_DURATION = 15
...
class AIReference(BaseModel):
    url: str = ""
    # Grok2API 也接受其媒体上传端点签发的 input_* file_id；其他协议继续只使用 url。
    file_id: str = ""
    name: str = ""
    role: str = ""
    kind: str = ""
...
async def probe_cangyuan_video_endpoint(client, base_url: str, api_key: str):
    """验证苍元的 Bearer 入口，不调用会产生费用的 POST /v1/videos。"""
    ok, result = await probe_openai_models_endpoint(client, base_url, api_key)
    result = dict(result or {})
    result["ok"] = bool(ok)
    if ok and isinstance(result.get("raw"), dict):
        grouped, ids = parse_upstream_models(result["raw"], CANGYUAN_VIDEO_PROTOCOL)
        result.update({
            "model_count": len(ids),
            "image_models": grouped["image"],
            "chat_models": grouped["chat"],
    ...
    result["protocol"] = CANGYUAN_VIDEO_PROTOCOL
    if ok:
        result["message"] = f"苍元视频协议可用：/v1/models 可达，{result.get('model_count') or 0} 个模型"
    else:
...
async def probe_grok2api_endpoint(client, base_url: str, api_key: str):
    ...
    result["ok"] = bool(ok)
    if ok and isinstance(result.get("raw"), dict):
        grouped, ids = parse_upstream_models(result["raw"], GROK2API_PROTOCOL)
        result.update({
            "model_count": len(ids),
            "image_models": grouped["image"],
            "chat_models": grouped["chat"],
...
async def probe_async_endpoint(payload: TestConnectionPayload):
    ...
    if is_cangyuan_video_protocol(protocol):
        try:
            async with httpx.AsyncClient(timeout=15) as client:
                probe = await probe_cangyuan_video_endpoint(client, base_url, api_key)
            return {
                "ok": bool(probe.get("ok")),
                "protocol": protocol,
                "status_code": probe.get("status") or 0,
    ...
    if is_grok2api_protocol(protocol):
        try:
            async with httpx.AsyncClient(timeout=20) as client:
                probe = await probe_grok2api_endpoint(client, base_url, api_key)
            return {
                "ok": bool(probe.get("ok")),
                "protocol": protocol,
                "status_code": probe.get("status") or 0,
...
def is_cangyuan_video_provider(provider):
    """是否选择了苍元视频协议。"""
    return is_cangyuan_video_protocol(provider_protocol(provider))

def is_cangyuan_video_route(provider, model=""):
...
async def wait_for_video_task(client, provider, task_id, submit_url="", model=""):
    ...
    while time.monotonic() < deadline:
        ...
        if status in VIDEO_TASK_FAILURE_STATUSES:
            ...
            contract_label = videos_contract_label(provider, model)
            if contract_label:
                raise HTTPException(status_code=502, detail=f"{contract_label}生成任务失败：{reason}")
...
async def sd2_public_reference_url(value, kind, index, label="chre3 视频"):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"{label}第 {index} 个{kind}参考素材无法转换为公网 URL：{exc.detail}",
        ) from exc
    except Exception as exc:
        raise HTTPException(
            status_code=400,
            detail=f"{label}第 {index} 个{kind}参考素材无法转换为公网 URL：{exc}",
        ) from exc
    parsed = urllib.parse.urlsplit(str(public_url or "").strip())
    if parsed.scheme.lower() not in {"http", "https"} or not parsed.netloc:
        raise HTTPException(
            status_code=400,
            detail=f"{label}第 {index} 个{kind}参考素材没有得到有效的公网 http(s) URL，请改用公网素材或配置媒体公网地址。",
        )
...
# ---- 苍元算力（ai.cangyuansuanli.cn）seedance 系视频：POST /v1/videos，GET /v1/videos/{id} ----
def cangyuan_video_duration(duration):
    try:
        value = int(duration)
    except Exception:
        value = 8
    return max(CANGYUAN_VIDEO_MIN_DURATION, min(CANGYUAN_VIDEO_MAX_DURATION, value))

def cangyuan_video_aspect_ratio(aspect_ratio="", size=""):
    value = str(aspect_ratio or "").strip() or str(size or "").strip()
    return value if value in CANGYUAN_VIDEO_ASPECT_RATIOS else "16:9"

def cangyuan_video_resolution(resolution=""):
    value = str(resolution or "").strip().lower()
    return value if value in CANGYUAN_VIDEO_RESOLUTIONS else CANGYUAN_VIDEO_DEFAULT_RESOLUTION

def cangyuan_reference_role(ref):
    if isinstance(ref, dict):
        return str(ref.get("role") or "").strip().lower()
    return str(getattr(ref, "role", "") or "").strip().lower()

def cangyuan_frame_pair(images):
    """首尾帧必须成对；只有两端都标注了角色才按首尾帧提交。"""
    first = next((ref for ref in (images or []) if cangyuan_reference_role(ref) in {"first_frame", "first"}), None)
    last = next((ref for ref in (images or []) if cangyuan_reference_role(ref) in {"last_frame", "last"}), None)
    return (first, last) if first is not None and last is not None else (None, None)

async def cangyuan_reference_url(value, kind, index, allow_data_uri=False):
    """苍元的参考图允许 data:image Base64 直传，参考视频/音频仍须公网直链。"""
    raw = str(value or "").strip()
    if not raw:
        return ""
    if allow_data_uri and raw.lower().startswith("data:"):
        return raw
...
async def build_cangyuan_video_request(payload, requested_model):
    ...
    if audio_values:
        body["reference_audios"] = [
            await cangyuan_reference_url(value, "音频", index)
            for index, value in enumerate(audio_values, 1)
        ]
    return body
...
async def generate_cangyuan_video(client, payload, provider, base_url, requested_model):
    ...
    body = await build_cangyuan_video_request(payload, requested_model)
    submit_url = video_submit_url_candidates(provider, base_url, requested_model)[0]
    response = await client.post(
        submit_url,
        headers=api_headers(provider=provider, model=requested_model),
    ...
    try:
        ...
        if looks_like_html_response(resp_text):
            detail = (
                "苍元视频接口返回了网页 HTML，而不是 JSON。请确认 API 设置里的 Base URL 是中转站接口根地址，"
                "例如 https://ai.cangyuansuanli.cn，而不是管理后台网页地址。"
            )
        else:
            detail = f"苍元视频接口返回非 JSON 响应（状态 {response.status_code}）：{resp_text}"
        raise HTTPException(status_code=502, detail=detail) from exc
    if not isinstance(raw, dict):
        raise HTTPException(status_code=502, detail=f"苍元视频接口返回了无法识别的响应：{str(raw)[:500]}")
    task_id = extract_task_id(raw) or raw.get("task_id") or raw.get("id")
    result = raw
    initial_task_data = raw.get("data") if isinstance(raw.get("data"), dict) else raw
    initial_status = str(initial_task_data.get("status") or initial_task_data.get("task_status") or "").upper()
    if initial_status in VIDEO_TASK_FAILURE_STATUSES:
        error = initial_task_data.get("error") if isinstance(initial_task_data.get("error"), dict) else {}
        reason = (
            initial_task_data.get("message")
            or error.get("message")
            or initial_task_data.get("error_code")
            or raw.get("message")
            or raw.get("error")
            or str(raw)
        )
        raise HTTPException(status_code=502, detail=f"苍元视频生成任务失败：{reason}")
    if task_id and not video_output_urls(raw):
        result = await wait_for_video_task(client, provider, task_id, submit_url, requested_model)
    urls = video_output_urls(result)
    if not urls:
...
try:
    _grok2api_poll_timeout = float(os.getenv("GROK2API_VIDEO_POLL_TIMEOUT", str(2 * 60 * 60 + 5 * 60)))
    GROK2API_VIDEO_POLL_TIMEOUT = (
        _grok2api_poll_timeout
        if math.isfinite(_grok2api_poll_timeout) and _grok2api_poll_timeout > 0
        else float(2 * 60 * 60 + 5 * 60)
    )
except (TypeError, ValueError):
...
async def wait_for_grok2api_video_task(client, provider, base_url, task_id, model):
    ...
    Grok2API 的后台视频任务最长约 2 小时；这里的超时只约束服务端后台
    工作，不再占用浏览器到本服务的长 HTTP 请求。
    """
    deadline = time.monotonic() + GROK2API_VIDEO_POLL_TIMEOUT
    delay = 5.0
    last_payload = {}
    while time.monotonic() < deadline:
        await asyncio.sleep(delay)
        url = grok2api.grok2api_video_task_url(base_url, task_id)
        try:
            response = await client.get(
                url,
                headers=api_headers(json_body=False, provider=provider, model=model),
                timeout=GROK2API_POLL_REQUEST_TIMEOUT,
            )
        except httpx.HTTPError as exc:
            print(f"[grok2api] 轮询网络错误，保留任务号继续重试：{str(exc) or type(exc).__name__}")
            delay = min(delay + 1.0, 10.0)
            continue
        try:
            raw = response.json() if response.text else {}
        ...
        if response.status_code in {408, 425, 429} or response.status_code >= 500:
            print(f"[grok2api] 轮询暂时失败 http={response.status_code}，保留任务号继续重试")
            delay = min(delay + 1.0, 10.0)
            continue
        if response.status_code == 404:
            detail = grok2api.grok2api_error_text(raw, getattr(response, "text", ""))
            raise HTTPException(status_code=502, detail=f"Grok2API 视频任务不存在：{detail}")
        if response.status_code >= 400:
            detail = grok2api.grok2api_error_text(raw, getattr(response, "text", ""))
            raise HTTPException(status_code=502, detail=f"Grok2API 视频任务查询失败：{detail}")
        if not isinstance(raw, dict):
            raise HTTPException(status_code=502, detail="Grok2API 视频任务查询返回了无法识别的响应。")
        last_payload = raw
        state, status = grok2api.grok2api_task_state(raw)
        if state == "success":
            if not grok2api.grok2api_video_result_urls(raw):
                raise HTTPException(status_code=502, detail="Grok2API 视频任务完成但没有返回 video.url。")
            return raw
        if state == "failed":
            raise HTTPException(
                status_code=502,
                detail=f"Grok2API 视频生成任务失败：{grok2api.grok2api_error_text(raw, status)}",
            )
        delay = min(delay + 0.5, 10.0)
    raise HTTPException(status_code=504, detail=f"Grok2API 视频任务超时：{last_payload or task_id}")

async def generate_grok2api_video(client, payload, provider, base_url, requested_model, on_task_id=None):
    api_key = grok2api_api_key(provider)
    body = await grok2api.build_grok2api_video_request(
        payload,
        requested_model,
        resolve_ref=make_grok2api_resolver(),
    )
    submit_url = grok2api.grok2api_video_submit_url(base_url)
    response = await client.post(
        submit_url,
        headers=api_headers(provider=provider, model=requested_model),
        json=body,
        timeout=GROK2API_SUBMIT_REQUEST_TIMEOUT,
    )
    if response.status_code >= 400:
        detail = grok2api_response_error_text(response)
        print(f"[video] Grok2API 上游失败 http={response.status_code} model={requested_model} detail={str(detail)[:300]}")
        raise HTTPException(status_code=response.status_code, detail=f"Grok2API 视频接口错误：{detail}")
    try:
        raw = response.json()
    except Exception as exc:
        resp_text = (response.text or "")[:500]
        if looks_like_html_response(resp_text):
            detail = "Grok2API 视频接口返回了网页 HTML，请确认 Base URL 是 API 根地址。"
        else:
            detail = f"Grok2API 视频接口返回非 JSON 响应（状态 {response.status_code}）：{resp_text}"
        raise HTTPException(status_code=502, detail=detail) from exc
    if not isinstance(raw, dict):
        raise HTTPException(status_code=502, detail=f"Grok2API 视频接口返回了无法识别的响应：{str(raw)[:500]}")

    task_id = grok2api.grok2api_task_id(raw)
    if not task_id:
        raise HTTPException(status_code=502, detail=f"Grok2API 视频接口没有返回 request_id：{str(raw)[:500]}")
    print(f"[grok2api] 视频任务已创建（已计费）task={task_id} model={body.get('model')}")
    if on_task_id:
        try:
            on_task_id(task_id)
        except Exception as exc:
            # 状态留痕失败不能改变已经提交的上游任务结果。
            print(f"[grok2api] 本地任务号留痕失败 task={task_id}: {exc}")
    state, status = grok2api.grok2api_task_state(raw)
    if state == "failed":
        raise HTTPException(
            status_code=502,
    ...
    result = raw if state == "success" else await wait_for_grok2api_video_task(
        client, provider, base_url, task_id, requested_model
    )
    ...
    content_url = grok2api.grok2api_video_content_url(base_url, task_id)
    local_url = await save_remote_video_to_output(
        content_url,
        prefix="grok2api_video_",
        extra_headers={"Authorization": bearer_auth_value(api_key)},
    ...
    if not _saved_ok(local_url):
        ...
        for candidate in result_urls:
            ...
            if _saved_ok(fallback):
                ...
                break
    if not _saved_ok(local_url):
        print(f"[grok2api] 成片下载失败 task={task_id} model={requested_model}")
        raise HTTPException(
            status_code=502,
            detail=(
                f"Grok2API 任务 {task_id} 已完成但成片下载失败。该任务已经提交并可能计费，"
                "请尽快使用任务号在 Grok2API 侧重试下载。"
            ),
        )
    return {"videos": [local_url], "task_id": task_id, "raw": result}
...
def enqueue_canvas_video_task(payload, provider, base_url, requested_model, task_type):
    ...
    with CANVAS_TASK_LOCK:
        CANVAS_TASKS[task_id] = {
            ...
            "status_code": 0,
            "upstream_task_id": "",
            "provider_id": provider.get("id") or payload.provider_id,
